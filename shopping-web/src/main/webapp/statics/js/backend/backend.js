...
function toAddProductCategory() {
    $.ajax({
        url: contextPath + "/sys/toAddProductCategory.action",
        method: "post",
...
function addProductCategory() {
    ...
    var type = $("#type").val();
    $.ajax({
        url: contextPath + "/sys/addProductCategory.action",
        method: "post",
        data: {
            name: name,
            type: type,
...
function queryProductCategoryList(obj, selectId) {
    ...
    $.ajax({
        url: contextPath + "/sys/queryProductCategoryList.action",
        method: "post",
        data: {
            parentId: parentId
        },
...
function toUpdateProductCategoryList(obj) {
    var id = $(obj).val();
    $.ajax({
        url: contextPath + "/sys/getProductCategoryById.action",
        data: {
            id: id
        },
        success: function (jsonStr) {
...
function modifyProductCategory() {
    ...
    var type = $("#type").val();
    $.ajax({
        url: contextPath + "/sys/modifyProductCategory.action",
        method: "post",
        data: {
            id: id,
            name: name,
...
function deleteProductCategory(id,type) {
 var bool=window.confirm("确认删除此分类信息么?");
	if(bool){
		$.ajax({
	        url: contextPath + "/sys/deleteProductCategory.action",
	        method: "post",
	        data: {
	            id: id,
	            type: type,
...
function deleteById(id) {
	...
	if(bool){
		$.ajax({
	        url: contextPath + "/sys/deleteProductById.action",
	        method: "post",
	        data: {
	            id: id
	        },
...
function addUser() {
    ...
    var password = $("input[name='password']").val();
    $.ajax({
        url: contextPath + "/sys/maintainUser.action",
        method: "post",
        data: {
           id: id,
           loginName: loginName,
        ...
        success: function (result) {
            if (result.status == 1) {
                window.location.href=contextPath+"/sys/queryUserList.action";
            }else{
            	showMessage(result.message);
            }
        }
...
function deleteUserId(id) {
	...
	if(bool){
		$.ajax({
	        url: contextPath + "/sys/deleteUserById.action",
	        method: "post",
	        data: {
	            id: id
	        },
